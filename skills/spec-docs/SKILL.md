...
每个功能点至少有一条异常行为需求覆盖失败路径。一经确认，R-n 永不复用与重排；作废条目保留编号并把标题改为 `[作废] <原标题>`。
...
`files` 两两不相交的任务才可并行；`depends_on` 只能引用编号更小的任务；每个 AC-n 至少被一个任务 refs 覆盖。`step` 仅允许 `test`、`impl`、`review`。结构化门禁对象只在 workflow 内存中传递，不写入 tasks、requirements 或额外报告。
...
lint 必须确认 requirements 恰好为第 1～7 节、R/AC 互相完整覆盖、AC 具有固定四行结构，并验证同目录两个 canonical v1 ledger。输出 FAIL 时不得进入 `confirmed`。
